...
use super::{
    app::{App, Overlay},
    components::{dialog_box, logo, output_block, planner_overlay, scanner_overlay, suggestion_list},
};
...
pub fn draw(f: &mut Frame, app: &App) {
    ...
    let fixed_bottom = spinner_h + dialog_h + suggestion_h;
    ...
    let clamped = (app.output_scroll as u16).min(max_scroll);
    ...
    if app.is_executing {
        f.render_widget(dialog_box::render_spinner_bar(app.elapsed), chunks[3]);
    }
    ...
    // Layout: border(1) + "> "(2) + text before cursor
    if !app.is_executing {
